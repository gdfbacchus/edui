...
const setInitLatencies = (urls) => {
  console.log("INIT LATENCIES...")
  let connectionManager;

  connectionManager = new Manager({url: urls[0], urls});
  let connectionCheckPromise = connectionManager.checkConnections();
  return Promise.all([connectionCheckPromise]).then((res => {
    if (connectionCheckPromise && res[0]) {
      let [latencies] = res;
      console.log("INIT latencies:", latencies);
...
//NEW VERSION
const willTransitionTo2 = (nextState, replaceState, callback) => {
  if(apiLatenciesCount===0) {
    setInitLatencies(filterAndSortURLs(apiLatenciesCount, apiLatencies))
      .then((res) => {
          console.log("INIT RES: ",res)
          processTransition(nextState, replaceState, callback)}
        )
      .catch((err) => {
        console.error("----- App. setInitLatencies error ----->", error, (new Error).stack);
      });
  } else {
    processTransition(nextState, replaceState, callback);
  }
...
const processTransition = (nextState, replaceState, callback) => {

  apiLatencies = ss.get("apiLatencies", {});
  console.log("SECOND apiLatencies: ",apiLatencies);
...
export default willTransitionTo2;

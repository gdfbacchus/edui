...
class GatewayStore {
    ...
    onFetchCoins({backer, coins, backedCoins, down} = {}) {
        if (backer && coins) {
            for(let i in coins){
              for(let prop in coins[i] ){
                  if(coins[i][prop]=== "true") {
                    coins[i][prop]= true;
                  } else if (coins[i][prop]=== "false") {
                    coins[i][prop]= false;
                    }
              }
            }

            //console.log("onFetchCoins backer: ",backer);
            //console.log("onFetchCoins coins: ",coins);
            this.coins = this.coins.set(backer, coins);
            // this.backedCoins = this.backedCoins.set(backer, backedCoins);//OL

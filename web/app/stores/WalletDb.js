...
class WalletDb extends BaseStore {
    ...
    decryptTcomb_PrivateKey(private_key_tcomb) {
        ...
        let pk=PrivateKey.fromBuffer(new Buffer(private_key_hex, 'hex'));
        return pk;
    }
    ...
    getPrivateKey(public_key) {
        ...
        //console.log("2 public_key: ",public_key);
        let private_key_tcomb = PrivateKeyStore.getTcomb_byPubkey(public_key)
        //console.log("3 private_key_tcomb: ",private_key_tcomb);
        if(! private_key_tcomb) return null
        let prKey = this.decryptTcomb_PrivateKey(private_key_tcomb);
        //console.log("4 prKey: ",prKey);
        return prKey;
    }
            ...
            return new Promise( (resolve, reject) => {
                ...
                let end = idb_helper.on_transaction_end(transaction).then( () => {
                    ...
                    if(unlock) {
                        ...
                        //console.log("3.0.0 INIT local_aes_private: ",local_aes_private)
        ...
        if (account) {
            ...
            try {
                ...
                if( unlock ) {
                    ...
                    //console.log("3.0.1 password: ",password);
                    //console.log("3.0.2 password_aes: ",password_aes);

                    let encryption_plainbuffer = password_aes.decryptHexToBuffer( wallet.encryption_key );
                    //console.log("3.0.3 encryption_plainbuffer: ",encryption_plainbuffer);
                    aes_private = Aes.fromSeed( encryption_plainbuffer );
                    //console.log("3.0.4 aes_private: ",aes_private);
                }
                return true;
            } catch(e) {
                console.error(e);
    ...
    changePassword( old_password, new_password, unlock = false ) {
        return new Promise( resolve => {
            ...
            if( unlock ) {
                aes_private = Aes.fromSeed( encryption_plainbuffer )
                //console.log("ch pass unlock aes_private: ",aes_private)
            } else {

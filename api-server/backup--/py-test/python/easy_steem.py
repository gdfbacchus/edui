...
class easySteem(easyInterface):
    ...
    def __transfers(self):
        ...
        while True:
            try:
                transfers = chain.stream(filter_by=['transfer'])
                for trf in transfers:
                    ...
                    if not all(key in trf.keys() for key in tr_keys):
                        continue
                    if trf['trx_id'] in trx_ids.values():
                        print("tx present", trf)
                        continue
                    ...
                    if trf['from'] == self.owner_account and amount[1] in self.confirmation.keys():
                        ...
                        trx_ids[datetime.now()] = trf['trx_id']
                ...
                for trx_time in list(trx_ids.keys()):
                    diff = datetime.now() - trx_time
                    if diff.days > 1:
                        trx_ids.pop(trx_time)
                pass

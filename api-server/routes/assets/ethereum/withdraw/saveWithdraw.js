...
var saveWithdraw = function (req, res, next) {
  logMessage = formatLogMsg('INFO', req.body, "ETH Save Wi Parsed request body", null, null, logContext);
  logger.log('info', logMessage);

  var easydexCode = makeEasydexCode(39);
  var startTime = getCurrentUTCtime();

  logger.log('info','ETH START DATE TO SAVE WITHDRAW: '+startTime);
  ...
  dbScripts.insertInitWithdrawData(req, res, easydexCode, startTime)
    .then( function(resp) {
      ...
      if(resp) {
        var msg ="ETH Withdraw Transaction was saved(inserted) successfully";
        ...
        errMsg = "ETH Withdraw Transaction wasn't saved, response of insertInitWithdrawData method is empty!";
    ...
    .catch(err => {
      errMsg = "ETH withdraw DB insertInitWithdrawData() failed!"

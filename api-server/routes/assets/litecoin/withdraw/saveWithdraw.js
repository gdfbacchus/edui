...
var saveWithdraw = function (req, res, next) {
  ...
  logMessage = formatLogMsg('INFO', req.body, "Start to save LTC Withdraw data", null, null, logContext);
  logger.log('info', logMessage);

  var calcAmount = req.body.amount;
  var easydexCode = makeEasydexCode(40);
  var startTime = getCurrentUTCtime();

  logMessage = formatLogMsg('INFO', null, "LTC Generated easydexCode", {easydexCode: easydexCode}, null, logContext);
  logger.log('info', logMessage);
  ...
  dbScripts.insertInitWithdrawData(req, res, calcAmount, easydexCode, startTime)
    .then( function(resp) {
      ...
      logMessage = formatLogMsg('INFO', null, "LTC Withdraw data was saved successfully", null, null, logContext);
      logger.log('info', logMessage);
      res.send(JSON.stringify({"status": 200, "error": null, "response": JSON.stringify(response)}))
    })
    .catch(err => {
      var errMsg = "LTC DB insertInitWithdrawData() failed!";

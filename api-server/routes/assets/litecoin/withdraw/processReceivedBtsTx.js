...
var processReceivedBtsTx = function (req, res, next) {
  ...
  try {
    ...
    logMessage = formatLogMsg('INFO', req.body, "LTC Parsed request body", json, null, logContext);
    logger.log('info', logMessage);
  }
  catch (err) {
    logMessage = formatLogMsg('ERROR', req.body, "LTC Given request cannot be parsed!", null, err, logContext);
  ...
  dbScripts.getInitialWithdrawData(res, json)
    .then( function(resp) {
      ...
      try{
        ...
        logMessage = formatLogMsg('INFO', null, "LTC dbScripts.getInitialWithdrawData() response", resJSONData, null, logContext);
        logger.log('info', logMessage);

      } catch (err){
        logMessage = formatLogMsg('ERROR', req.body, "LTC Result data from dbScripts.getInitialWithdrawData(), cannot be parsed.", null, err, logContext);
        logger.log('error', logMessage);
        res.status(500);
      ...
      if(resJSONData && resJSONData.length === 1){
        ...
        dbScripts.updateWithdrawStatus(res, resultObj.easydex_code, "pending")
          .then(function(updateResponse){
            logger.log('info', "LTC Transaction was updated with [pending] status.");
            //res.send(JSON.stringify( { "status": 200, "error": null, "response": "Transaction was updated with [pending] status." } ) );
          })
          .catch(function(err){
            console.log("ERROR: ", err);
            var errMsg = "LTC DB updateWithdrawStatus() failed. Transaction was NOT updated with [pending] status.";
        ...
        logMessage = formatLogMsg('INFO', null, "LTC Validation data", validationData, null, logContext);
        logger.log('info', logMessage);
        ...
        if(isValidAmount &&
          ...
          logMessage = formatLogMsg('INFO', null, "LTC Validation is passed.", null, null, logContext);
          logger.log('info', logMessage);

          minerFee = minerFee.toFixed(8);
          amount = amount.toFixed(8);

          logMessage = formatLogMsg('INFO', null, "LTC Miner fee and amount to send", {minerFee: minerFee, amount:amount }, null, logContext);
          logger.log('info', logMessage);
          ...
          manager.setTxFee(minerFee, function(err, result) {
            if (err) {
              logMessage = formatLogMsg('ERROR', req.body, "LTC setTxFee() failed.", null, err, logContext);
              logger.log('error', logMessage);
              res.status(500);
              res.send(JSON.stringify({"status": 500, "error": "error", "response":""}));
              return;
            ...
            if(JSON.parse(result)===true) {
              logMessage = formatLogMsg('INFO', null, "LTC setTxFee() result", result, null, logContext);
              logger.log('info', logMessage);

              // 5 STEP - Send to address
              manager.sendToAddress( addrSendTo, amount, function(err, txid){
                if (err) {
                  logMessage = formatLogMsg('ERROR', req.body, "LTC sendToAddress() failed.", null, err, logContext);
                  logger.log('error', logMessage);
                  res.status(500);
                  res.send(JSON.stringify({"status": 500, "error": "error", "response":""}));
                  return;
                ...
                logMessage = formatLogMsg('INFO', null, "LTC sendToAddress() result->txid", txid, null, logContext);
                logger.log('info', logMessage);
                ...
                dbScripts.updateWithdrawData(res, resultObj.easydex_code, "empty", txid, "sent")
                  .then(function(addrResponse){
                    logMessage = formatLogMsg('INFO', null, "LTC WITHDRAW DB DATA WAS SUCCESSFUL UPDATED :)", null, null, logContext);
                    logger.log('info', logMessage);
                    logger.log('info', "LTC Transaction was sent successfully.");
                    res.send(JSON.stringify( { "status": 200, "error": null, "response": "LTC Transaction was sent successfully." } ) );
                    return;
                  })
                  .catch(function(err){
                    var errMsg = "LTC DB updateWithdrawData() fails.";
                    logMessage = formatLogMsg('ERROR', req.body, errMsg, null, err, logContext);
                    logger.log('error', logMessage);
                    res.status(500);
                    res.send(JSON.stringify({"status": 500, "error": "error", "response":""}));
          ...
          logMessage = formatLogMsg('ERROR', req.body, "LTC Validation of received bts json-data is not passed.", validationData, null, logContext);
          logger.log('error', logMessage);
          res.status(500);
          res.send(JSON.stringify({"status": 500, "error": "error", "response":""}));
          return;
        ...
        // Not found DB result
        logMessage = formatLogMsg('ERROR', req.body, "LTC dbScripts.getInitialWithdrawData() method Not found corresponding DB record for the request.", null, null, logContext);
        logger.log('error', logMessage);
        res.status(500);
        res.send(JSON.stringify({"status": 500, "error": "error", "response":""}));
        return;
    ...
    .catch(err => {
      console.log("ERROR: ", err);
      var errMsg = "LTC DB getInitialWithdrawData() fails!";

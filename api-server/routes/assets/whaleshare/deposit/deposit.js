...
var constants = require('../constants/wls-constants');
...
var deposit = (req, res, next) => {
  var req_json = null;
  req_json = req.body;
  logMessage = formatLogMsg('INFO', null, "WLS Deposit Parsed request body", req_json, null, logContext);
  ...
  */
  ...
  if(!easydexConfirmed) {
    errMsg = "WLS Deposit CONFIRM STATUS WAS NOT RECEIVED! easydex-confirm = ["+easydexConfirmed+"]";
  ...
  dbScripts.getDepositTransactionRecord(res, txid).then(function(result){
    ...
    if(parsedResult && parsedResult.length > 0 && parsedResult.length === 1) {
      ...
      if (easydexConfirmed == "true") {
        ...
        if(!req_json.signatures) {
          errMsg = "WLS Deposit Transaction cannot be updated, easydexConfirmed=["+easydexConfirmed+"] !";
        ...
        dbScripts.updateDepositTxStatus(res, txid, current_timestamp, signatures, status)
          .then(function (updated) {
            if (updated) {
              var msg = "WLS Deposit Transaction was updated successfully status:[" + status + "]";
              ...
              errMsg = "WLS Transaction wasn't updated, response of updateDepositTxStatus method is empty!";
          ...
          .catch(function (err) {
            errMsg = "WLS Deposit Transaction wasn't updated, DB method updateDepositTxStatus failed!";
            logMessage = formatLogMsg('ERROR', req.body, errMsg, null, err, logContext);
            logger.log('error', logMessage);
            res.status(500);
            res.send(JSON.stringify({"status": 500, "error": errMsg, "response": ""}));
    ...
    //IF THE RECORD NOT EXISTS
      ...
      if(status === "sent") {
        var msg = "WLS Deposit Transaction was not inserted status:[" + status + "], it MUST be [pending]";
        logger.log('info', msg);
        res.status(500);
      ...
      if(aaObj) {
        ...
        errMsg = "WLS Deposit Transaction cannot parse amount and asset!";
        logMessage = formatLogMsg('ERROR', req.body, errMsg, req_json.amount, null, logContext);
        logger.log('error',logMessage);
        res.status(500);
        res.send(JSON.stringify({"status": 500, "error": "error", "response":""}));
      ...
      dbScripts.insertDepositTransaction(res, dataToInsert)
        .then(function(result){
          if(result) {
            var msg ="WLS Deposit Transaction was inserted successfully!";
            ...
            errMsg = "WLS Deposit Transaction wasn't inserted, response of insertDepositTransaction method is empty!";
        ...
        .catch(function(err){
          errMsg = "WLS Deposit Transaction wasn't inserted!";
          logMessage = formatLogMsg('ERROR', req.body, errMsg, null, err, logContext);
          logger.log('error',logMessage);
          res.status(500);
          res.send(JSON.stringify({"status": 500, "error": errMsg, "response":""}));
    ...
    errMsg = "WLS Deposit Transaction DB getDepositTransactionRecord method failed!"
    logMessage = formatLogMsg('ERROR', req.body, errMsg, null, err, logContext);
    logger.log('error',logMessage);
    res.status(500);

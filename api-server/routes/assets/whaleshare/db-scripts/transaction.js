  ...
  return new Promise((resolve, reject) => {
    ...
    res.locals.connectionPool.getConnection(function(err, connection) {
      ...
      var queryCheckTxDataExists =
        "SELECT id, wls_trx_id, status " +
        "FROM `wls_deposit` " +
        "WHERE wls_trx_id=" + connection.escape(txid) + " ;";
...
exports.insertDepositTransaction = function (res, data) {
  return new Promise((resolve, reject) => {
    res.locals.connectionPool.getConnection(function(err, connection) {
      ...
      var insertQuery = "INSERT INTO `wls_deposit`(`wls_trx_id`, `_id`, `send_from`, `send_to`, `amount`, `asset`, `memo`, `wls_timestamp`, `date_received`, `status`) " +
        "VALUES ("
...
exports.insertInitWithdrawData = function (req, res, easydexCode, startTime) {
  return new Promise((resolve, reject) => {
    res.locals.connectionPool.getConnection(function(err, connection) {
      ...
      var insertQuery = "INSERT INTO `wls_withdrawal`(" +
        "`from_account`, " +
        "`to_account`, " +
        "`amount`, " +
        "`asset`, " +
...
exports.updateWithdrawStatus = function (res, easydexCode, status) {
  return new Promise((resolve, reject) => {
    res.locals.connectionPool.getConnection(function(err, connection) {
      ...
      var updateQuery = "UPDATE `wls_withdrawal` SET "
        + " `status`=" + connection.escape(status)
...
exports.updateWithdrawTx = function (res, data ) {
  return new Promise((resolve, reject) => {
    res.locals.connectionPool.getConnection(function(err, connection) {
      ...
      var updateQuery = "UPDATE `wls_withdrawal` SET " + strToUpdate + " WHERE `easydex_code`=" + connection.escape(code) + ";";

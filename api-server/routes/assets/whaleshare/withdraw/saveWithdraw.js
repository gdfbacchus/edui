...
var saveWithdraw = function (req, res, next) {
  logMessage = formatLogMsg('INFO', req.body, "WLS Save Wi Parsed request body", null, null, logContext);
  ...
  dbScripts.insertInitWithdrawData(req, res, easydexCode, startTime)
    .then( function(resp) {
      ...
      if(resp) {
        var msg ="WLS Withdraw Transaction was saved(inserted) successfully";
        ...
        errMsg = "WLS Withdraw Transaction wasn't saved, response of insertInitWithdrawData method is empty!";
    ...
    .catch(err => {
      errMsg = "WLS withdraw DB insertInitWithdrawData() failed!"

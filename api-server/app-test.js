...
//console.log("TEST API db settings: ",dbSettings);
...
app.use(function (req, res, next) {
  ...
  logger.log('info',"REQUEST BODY/QUERY: ");
  logger.log('info',!req.body ? !req.query ? "---": req.query : req.body );
  logger.log('info',"====================================================================");
  next()
});
...
var server = app.listen(3001,function(err){
  ...
  //console.log("TEST EasyDex API Listening on port %s",server.address().port);
  logger.log('info','STAGING API Listening on port: ' + server.address().port);
